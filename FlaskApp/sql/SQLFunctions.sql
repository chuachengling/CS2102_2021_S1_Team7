...
BEGIN 
  RETURN QUERY(
    SELECT DISTINCT a.pet_name FROM Looking_After a
    WHERE a.po_userid = userid -- may need to see whether pet is dead or not
  );
END;
$func$
LANGUAGE plpgsql; 
...
RETURNS TABLE (hp INT) AS 
$func$
BEGIN 
RETURN QUERY(
  SELECT b.hp FROM Users b
  WHERE b.userid = find_hp.userid
);
END;
$func$
LANGUAGE plpgsql;
...
BEGIN
RETURN QUERY(
	SELECT a.ct_userid AS name, a.pet_name, a.start_date, a.end_date
	FROM Looking_After a
	WHERE (a.po_userid = pastTransactions.userid OR a.ct_userid = pastTransactions.userid) AND status = 'Completed'
	);
END;
$func$
LANGUAGE plpgsql;
